...
> Идентификаторы устройства (IP, MAC, пароль) в отчёте **анонимизированы** —
> заменены на плейсхолдеры (`192.168.1.100`, `AA:BB:CC:DD:EE:FF`). Методология и
> ход работ — без изменений.
...
На руках — два захвата PCAPdroid (переключения режимов) и живой бар на `192.168.1.100`.
...
Для бара (wired MAC `AA:BB:CC:DD:EE:FF`):
`password = QUFCQkNDRERFRUZGS2xpcHNjaFN1cHBvcnQhITg4`. Вписано в `auth.py`.
...
1. **Нет BLE** — приложению нужен Bluetooth, чтобы достать MAC и увидеть бар; у эмулятора
   BLE нет.
2. **Нет mDNS через NAT** — пинг до `192.168.1.100` проходит, но Cast/eureka-discovery
...
строки, режим AES, ключ HMAC) и валидировать гипотезы прямо против `192.168.1.100`.
...
192.168.1.100/32` (только бар через туннель, discovery остаётся на прямом Wi-Fi),
...
- **Android-эмулятор (Pixel_8_API_35)** — Frida **РАБОТАЕТ** (`[HB] clock_gettime fired` —
  ...
  (Frida на приложении его не ловит). ISCP-запрос на `192.168.1.100:60128` — без ответа.
