...
2. **Большинство записей `setData` теперь требуют авторизации** (`settings:/webserver/authMode = setData`). Защищённые команды отвечают `401 Forbidden` с заголовком `WWW-Authenticate: HMAC_SHA256_AES256`.
...
| Громкость, без звука (Mute) | ✅ Работает (без авторизации) |
| Вход, звуковой режим, ночной/диалог, бас/средние/высокие, пресет EQ, Dirac, сабвуфер и surround-уровни, питание | ❌ `401` — заблокировано авторизацией прошивки |

Текущий статус по каждой команде виден в **Download diagnostics** (раздел `command_health`, добавлен в v2.4.2).

### Статус исправления

🚧 **Готового решения пока нет — в работе.** Для возврата полного управления нужно реализовать подпись запросов `HMAC_SHA256_AES256`. Секрет подписи провижинит официальное приложение, и он ещё устанавливается (пользовательский пароль не задан, веб-интерфейс устройства отключён, поэтому вариант с пустым паролем не работает). До этого чтение и громкость/Mute остаются рабочими.
